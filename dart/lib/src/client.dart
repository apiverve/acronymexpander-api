...
class AcronymexpanderValidationException implements Exception {
  final List<String> errors;

  AcronymexpanderValidationException(this.errors);

  @override
  String toString() => 'AcronymexpanderValidationException: ${errors.join("; ")}';
}
...
/// Acronym Expander API client.
///
/// For more information, visit: https://apiverve.com/marketplace/acronymexpander?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [acronym] (required) - The acronym to expand (max 20 characters) [maxLength: 20]
/// * [context] - Optional context to help determine the correct meaning
class AcronymexpanderClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'acronym': ValidationRule(type: 'string', required: true, maxLength: 20),
    'context': ValidationRule(type: 'string', required: false),
  };

  AcronymexpanderClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/acronymexpander',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AcronymexpanderValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AcronymexpanderValidationException(errors);
    }
  }

  /// Execute a request to the Acronym Expander API.
  ///
  /// Parameters are validated before sending the request.
  Future<AcronymexpanderResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AcronymexpanderException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AcronymexpanderResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AcronymexpanderException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AcronymexpanderException('Resource not found');
      } else {
        throw AcronymexpanderException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AcronymexpanderException) rethrow;
      throw AcronymexpanderException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Acronym Expander API client.
class AcronymexpanderException implements Exception {
  final String message;

  AcronymexpanderException(this.message);

  @override
  String toString() => 'AcronymexpanderException: $message';
}
