...
/// Main API client for Acronym Expander
public class AcronymExpanderAPIClient {
    ...
    public init(apiKey: String) {
        ...
        self.apiKey = apiKey
        self.baseURL = "https://api.apiverve.com/v1/acronymexpander"
    ...
    public func execute(parameters: [String: Any]? = nil, completion: @escaping (Result<APIResponse, AcronymExpanderAPIError>) -> Void) {
        executeGet(parameters: parameters, completion: completion)
    }

    private func executeGet(parameters: [String: Any]?, completion: @escaping (Result<APIResponse, AcronymExpanderAPIError>) -> Void) {
        ...
        guard let url = urlComponents?.url else {
            completion(.failure(AcronymExpanderAPIError.invalidURL))
            return
        }
    ...
    private func performRequest(_ request: URLRequest, completion: @escaping (Result<APIResponse, AcronymExpanderAPIError>) -> Void) {
        let task = session.dataTask(with: request) { data, response, error in
            if let error = error {
                completion(.failure(AcronymExpanderAPIError.networkError(error)))
                return
            }

            guard let httpResponse = response as? HTTPURLResponse else {
                completion(.failure(AcronymExpanderAPIError.invalidResponse))
                return
            }

            guard let data = data else {
                completion(.failure(AcronymExpanderAPIError.noData))
                return
            }
            ...
            if httpResponse.statusCode != 200 {
                do {
                    ...
                    completion(.failure(AcronymExpanderAPIError.apiError(statusCode: httpResponse.statusCode, message: errorResponse.error ?? "Unknown error")))
                } catch {
                    completion(.failure(AcronymExpanderAPIError.httpError(statusCode: httpResponse.statusCode)))
                }
                return
            }
            ...
            do {
                ...
                completion(.failure(AcronymExpanderAPIError.decodingError(error)))
            }
        }
...
extension AcronymExpanderAPIClient {
    ...
    /// - Throws: AcronymExpanderAPIError on failure
